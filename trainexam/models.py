...
class Qts_Library(models.Model):
    ...
    '''备选答案choosableanswer字段：保存为一个长字符串。
        判断题直接为 T/F、对/错、正确、错误、True、False、1、0几样，以判断 T、对、正确、OK、1为主;
    ...
    choosableanswer = models.TextField() #备选答案,当是判断题是就是该题的答案
    ...
    qts_level = models.IntegerField(default = 0) #题目级别（难度,0 1 2 3）
...
class Qts_Testpapers(models.Model):
    '''测试考试试卷列表'''
    userid = models.ForeignKey(Qts_Userid,on_delete = models.CASCADE)
